...
@export
def cifar_pyramidnet_110_48(pretrained=False, **kwargs):
    assert not pretrained
    model = PyramidNet(depth=110,
                       alpha=48, bottleneck=False,**kwargs)
    return model
...
class PyramidNet(nn.Module):
    def __init__(self, depth, alpha, num_classes=1000, bottleneck=False):
        super().__init__()
        self.inplanes = 16
        if bottleneck:
            n = int((depth - 2) / 9)
        ...
        self.avgpool = nn.AvgPool2d(8)
        self.fc1 = nn.Linear(self.final_featuremap_dim, num_classes)
        self.fc2 = nn.Linear(self.final_featuremap_dim, num_classes)
